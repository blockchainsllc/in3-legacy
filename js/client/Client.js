...
class Client {
    constructor(config) {
        this.defConfig = Object.assign({ minDeposit: 0, requestCount: 3, contract: '0xF88e75205BcD029C897700E8ad03050C78611A37', chainId: '0x2a', nodeList: [
                {
                    address: '0x01',
                    chainIds: ['0x2a'],
    ...
    async updateNodeList() {
        ...
        const nodes = await this.send(req, null, { minDeposit: 0 });
    ...
    async call(request, config) {
        return this.send(request, null, { minDeposit: 0 })
            .then(_ => _.result + '');
    }
    /**
     * sends a request.
    ...
    send(request, callback, config) {
        const p = this.sendIntern(Array.isArray(request) ? request : [request], config ? Object.assign({}, this.defConfig, config) : this.defConfig);
        if (callback)
            p.then(_ => callback(null, Array.isArray(request) ? _ : _[0]), callback);
        else
            return p.then(_ => Array.isArray(request) ? _ : _[0]);
    }
    async sendIntern(requests, conf, prevExcludes) {
        // check nodeList
...
let idCount = 1;
function prepareCall(contract, methodHash, params) {
    return {
        method: 'eth_call',
        params: [{
                to: contract,
 ...
 */
async function mergeResults(request, responses, conf) {
    ...
    if (Object.keys(groups).length > 1) {
        ...
        if (conf.nodeAuthorities && conf.nodeAuthorities.length) {
            const aconf = Object.assign({}, conf, { nodeList: conf.nodeAuthorities.map(a => conf.nodeList.find(_ => _.address === a)).filter(_ => _) });
 ...
 */
async function handleRequest(request, node, conf, excludes) {
    ...
    try {
        ...
        const res = await axios_1.default.post(node.url, request, { timeout: conf.timeout || 1000 });
 ...
 */
function getNodes(config, count, excludes) {
    const now = Date.now();
    // prefilter for chain && minDeposit && excludes
    const nodes = config.nodeList.filter(n => n.chainIds.indexOf(config.chainId) >= 0 && // check chain
